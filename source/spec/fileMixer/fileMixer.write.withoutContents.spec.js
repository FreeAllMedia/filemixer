...
describe("fileMixer.write() (without contents)", () => {
	let fileMixer,
			path,
			virtualFile,
			writtedDirectoryStats,
			temporaryDirectory;
	...
	beforeEach(done => {
		...
		.write((error, file) => {
			writtedDirectoryStats = fileSystem.statSync(file.path);
	...
	it("should write a directory at the designated path", () => {
		writtedDirectoryStats.isDirectory().should.be.true;
	});
